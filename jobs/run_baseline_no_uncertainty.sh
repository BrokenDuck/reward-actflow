#!/bin/bash
# Launch baseline: no uncertainty (no DPS guidance, still filters to valid data)
# Usage: bash jobs/run_baseline_no_uncertainty.sh
...
for FT_STEPS in "${FT_STEPS_LIST[@]}"; do
    for SEED in "${SEEDS[@]}"; do
        FOLDER="$BASE_DIR/baseline_no_uncertainty_t${TIMESTEP}_dps${DPS_WEIGHT}_ftsteps${FT_STEPS}_ftmin${FT_MIN_DS}_seed${SEED}"
        mkdir -p "$FOLDER"
        ...
        sbatch --dependency=afterok:$WARMUP_JOB \
            ...
            jobs/geom_ensemble_no_uncertainty.sh "$TIMESTEP" "$DPS_WEIGHT" "$FT_STEPS" "$BASE_DIR" "$WARMUP_CACHE_DIR" "$ALPHA_REG" "$SEED" "$FT_MIN_DS"
    done
done
