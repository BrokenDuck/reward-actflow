...
class TaskDirectedConfig:
    ...
    def construct_from_args(args: argparse.Namespace | dict) -> "TaskDirectedConfig":
        ...
        config_fields = {f.name for f in TaskDirectedConfig.__dataclass_fields__.values()}
        clean_kwargs = {}
        ...
        return TaskDirectedConfig(**clean_kwargs)


class TaskDirected(Generic[D]):
    def __init__(
        self,
        ...
        reward_entry = reward_registry.get(config.reward)
        reward = reward_entry.instantiate(**config.reward_kwargs)
