...
class Diffusion(L.LightningModule):
    ...
    def sample_finetuned_with_rnd(self, args, reward_model, pretrained, eps=1e-5):
        ...
        # If no valid peptides were generated, return the full batch with
        # zero rewards so the training loop can continue without crashing.
        # Must short-circuit BEFORE calling reward_model — passing an empty
        # list to the GP/ensemble uncertainty reward triggers a max() of an
        # empty sequence inside extract_embeddings.
        if len(valid_log_rnd) == 0:
            print("WARNING: No valid peptides in batch — returning full batch with zero reward")
            zero_rewards = torch.zeros(B, device=self.device)
            return x_rollout, log_rnd, zero_rewards
    ...
    def sample_finetuned(self, args, reward_model, batch_size=None, dataframe=False, eps=1e-5, return_scalars=False):
        ...
        if (len(validSequences) != 0):
            ...
            # Build a dict of per-objective score arrays keyed by name
            obj_names = reward_model.score_func_names if hasattr(reward_model, 'score_func_names') else [f'obj_{i}' for i in range(len(average_scores))]
            scores_dict = {name: average_scores[i] for i, name in enumerate(obj_names)}
            scalar_rewards = np.sum(score_vectors, axis=-1)
        else:
            obj_names = reward_model.score_func_names if hasattr(reward_model, 'score_func_names') else []
            scores_dict = {name: [0.0] for name in obj_names}
            scalar_rewards = [0.0]
            
        scalar_rewards = torch.as_tensor(scalar_rewards, dtype=torch.float32, device=self.device)
        
        if dataframe: 
            df = pd.DataFrame({"Peptide Sequence": validSequences if len(validSequences) else [""]})
            for name in scores_dict:
                df[name] = scores_dict[name] if len(validSequences) else [0.0]
            if return_scalars:
                return x_rollout, scalar_rewards, valid_fraction, df
            return x_rollout, scores_dict, valid_fraction, df
        
        if return_scalars:
            return x_rollout, scalar_rewards, valid_fraction
        return x_rollout, scores_dict, valid_fraction
    ...
    def generate_from_masked(self, num_samples=None, seq_length=None, sample_steps=128, eps=1e-5):
        ...
        for i in range(sample_steps):
            ...
            result = self.single_reverse_step(z, t, dt)
            # single_reverse_step returns (log_p, x_next)
            z = result[1] if isinstance(result, tuple) else result
    ...
    """SAMPLING"""
    @torch.no_grad()
    def _sample(self, num_steps=None, eps=1e-5, x_input=None, batch_size=None, seq_length=None):
        """ 
            Generate samples
        """
        if batch_size is None:
        ...
        if x_input is not None:
            ...
            x = self.sample_prior(batch_size_per_gpu, seq_length).to(self.device)
        ...
        for i in range(num_steps):
            ...
            if self.sampler == 'ddpm':
                _log_p, x = self.single_reverse_step(x, t, dt)
                x = x.to(self.device)
    ...
    def restore_model_and_sample(self, num_steps=None, eps=1e-5, batch_size=None, seq_length=None):
        """Generate samples from the model.
        
        Args:
            num_steps: Number of diffusion denoising timesteps.
                       Defaults to config.sampling.steps.
            eps:       Minimum timestep.
            batch_size: Number of sequences to generate.
                        Defaults to config.eval.perplexity_batch_size.
            seq_length: Token sequence length.
                        Defaults to config.model.length.
        """
        self.backbone.eval()
        self.noise.eval()
        samples = self._sample(num_steps=num_steps, eps=eps, batch_size=batch_size, seq_length=seq_length)
        self.backbone.train()
        self.noise.train()
        return samples
