...
class SampleCollection:
    ...
    def load_samples(self, device: torch.device):
        ...
        self.problem_setup = problem_setups[exp_args["problem_setup"]](
...
def get_pareto_front(
    points: np.ndarray, x_mode: str = "min", y_mode: str = "min"
) -> np.ndarray:
...
def plot_pareto(
    collections: list[SampleCollection], metrics: list[str], output_dir: Path
):
    ...
    for x_metric in metrics:
        for x_mode in ["min", "max"]:
            fig, ax = plt.subplots(
                len(metrics), 2, figsize=(10, 5 * len(metrics)), sharey="row"
            )
                ...
                for j, y_mode in enumerate(["min", "max"]):
                    for coll in collections:
                        ...
                        points = np.column_stack(
                            [df[x_metric].to_numpy(), df[y_metric].to_numpy()]
                        )
                        # Drop rows with NaN
                        ...
                        pareto_front = get_pareto_front(
                            points, x_mode=x_mode, y_mode=y_mode
                        )
                        ax[i][j].plot(
                            pareto_front[:, 0],
                            pareto_front[:, 1],
...
def plot_property_profiles(
    ...
    for coll, emb in zip(collections, all_embeds):
        ...
        ax.contourf(
            xx, yy, zz, levels=[alpha_density, zz.max()], alpha=0.3, colors=[coll.color]
        )
        ...
        for coll in collections:
            ...
            for n in ns:
                ...
                for _ in range(repeats):
                    ...
                    div = coll.problem_setup.compute_metrics(
                        winning_samples, coll.kwargs
                    )
            ...
            for col_idx, div_name in enumerate(diversity_metric_names):
                ...
                # Annotate n values
                for xi, yi, ni in zip(
                    div_metrics_by_n[div_name], mean_metric_by_n, ns_used
                ):
                    ax.annotate(
                        str(ni),
                        (xi, yi),
...
class RunAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if len(values) < 3:
            parser.error(
                "--run requires at least 3 arguments: FOLDER SAMPLE_FOLDER LABEL [COLOR]"
            )
...
def main():
    ...
    for i, (folder, sample_folder, label, color) in enumerate(args.runs):
        ...
        collections.append(
            SampleCollection(Path(folder), Path(sample_folder), label, c)
        )
