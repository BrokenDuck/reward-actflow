...
from diffusiongym.utils import identity_fn, temporary_workdir
from reward_actflow.molecules.rewards.utils import graph_to_mols, is_not_fragmented, is_valid, safe_mmff_relax
from reward_actflow.molecules.types import DDGraph


class XTBReward(Reward[DDGraph]):
    ...
    def __call__(self, sample: DDGraph, latent: DDGraph, **kwargs) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for idx, res in zip(valid_indices, xtb_results, strict=False):
            if res is None:
                continue
...
def parallel_xtb(mols: list[Chem.Mol]) -> list[XTBResult | None]:
    ...
    with temporary_workdir():
        ...
        subprocess.run(
            f"parallel -j {ncpus} 'xtb {{}} --hess --parallel 1 --namespace {{/.}} --json > {{/.}}.out 2>&1' ::: *.xyz",
            shell=True,
            check=False,
        )
