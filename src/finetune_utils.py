...
def loss_wdce(policy_model, log_rnd, x, num_replicates=16, weight_func=lambda l: 1/l, eps=1e-3, centering=False, uniform_weights=False):
    r"""
    ...
    uniform_weights: if True, use constant 1/B weights instead of softmax(log_rnd)
    """
    ...
    if uniform_weights:
        batch_weights = torch.ones(x.shape[0], device=x.device, dtype=torch.float32) / x.shape[0]
    else:
        batch_weights = log_rnd.detach().softmax(dim=-1) # [B]
        if centering:
